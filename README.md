# Studio Player for foobar2000

밝은 카드 기반 UI로 foobar2000을 바꾸는 패널 스크립트입니다.
`apple-music.js` **파일 하나**가 화면 전체를 그립니다.

> **[→ 라이브 미리보기](https://claude.ai/code/artifact/0a4426bb-fc4f-4960-b85a-b4fabc8d4355)**
> — 설치 전에 브라우저에서 눌러보세요. 저장소의 `preview.html`과 같은 파일이며,
> 이 스크립트를 캔버스 위에서 그대로 실행합니다.

오프화이트 배경에 흰 카드, 그리고 **재생 중인 곡 한 줄만 검은 알약**으로 강조하는 구성입니다.
색은 앨범아트에서만 옵니다.

| 영역 | 내용 |
|------|------|
| 상단 바 | 로고, 재생목록 이름과 곡 수, **검색**, 설정(톱니) |
| 왼쪽 | 앨범아트 카드와 트랜스포트 (셔플·이전·재생·다음·반복) |
| 오른쪽 | 재생목록 제목, `N Songs · 총 길이`, 번호가 붙은 트랙 리스트 |
| 하단 바 | **재생목록 칩**, 경과·탐색바·총 길이, 볼륨, **정렬** |

### 패널 모드

| 모드 | 그리는 것 |
|------|-----------|
| `all` | **위 네 영역 전부** — 대부분 이것만 쓰면 됩니다 |
| `art` | 앨범아트 카드 + 트랜스포트만 |
| `list` | 트랙 리스트만 |
| `bar` | 하단 바만 |
| `auto` | 패널 크기를 보고 자동 선택 (기본값) |

> **Default UI는 영역이 처음에 하나뿐입니다.** 그래서 기본은 그 하나에 전부 그리는 `all`입니다.
> 영역을 쪼개지 않아도 됩니다.

테마는 **라이트**(기본)와 **다크** 두 가지입니다. 배경에는 재생 중인 앨범아트를
블러 처리해(`img.StackBlur`) 아주 옅게 깔아, 곡마다 화면 색조가 미묘하게 달라집니다.
...
- **테마** — 라이트 / 다크
- **패널 모드** — 자동 / 전체 / 앨범아트 + 컨트롤 / 트랙 리스트 / 하단 바
...
| 속성 | 기본값 | 설명 |
|------|--------|------|
| `Studio.Mode (auto\|all\|art\|list\|bar)` | `auto` | 패널 모드 |
| `Studio.Theme (light\|dark)` | `light` | 테마 |
| `Studio.Scale %` | `100` | 고DPI 화면에서 키우기 (예: 150) |
| `Studio.List row height` | `46` | 트랙 리스트 행 높이 |
| `Studio.Font` | `Pretendard, ...` | 쉼표로 나열한 폰트 우선순위. **설치된 첫 번째 폰트**를 사용 |

> 속성 값을 바꾼 뒤에는 패널을 새로고침해야 반영됩니다
> (`Configure...` 창을 열고 `Apply`를 다시 누르거나 foobar2000 재시작).
>
> 이미 패널을 만든 뒤라면 **속성에 저장된 예전 값이 그대로 남습니다.** 스크립트를 새로
> 붙여넣어도 폰트가 안 바뀐다면, 패널 속성에서 `Studio.Font` 값을 직접 고치세요.
...
취향에 따라 `Studio.Font` 속성 값을 바꿔 다른 고딕을 쓸 수도 있습니다.
...
| 상단 바 돋보기 | **검색** &mdash; 누르면 입력창이 열리고, 타이핑하면 제목·아티스트·앨범으로 실시간 필터. `Esc`로 닫기 |
| 하단 바 정렬 | 제목 / 아티스트 / 앨범 / 길이 / 파일 경로 순, **무작위로 섞기**. 정렬 전에 `UndoBackup`을 부르므로 `Edit` → `Undo`로 되돌릴 수 있습니다 |
| 상단 바 톱니 | **foobar2000 설정 창** (`Ctrl+P`와 같은 것) |
| 하단 왼쪽 칩 | 재생목록 목록 &mdash; 고르면 전환, **`＋ 새 재생목록…`** 으로 그 자리에서 생성 |
| 상단 바 대기열 / 재생목록 이름 | 재생목록 전환 메뉴 |
