...
function paintPage(gr, x, y, w, h) {
    ...
    var bw = Math.round(window.Width), bh = Math.round(window.Height * 0.55);
    var img = getBlob(bw, bh);
    if (img && y < bh) {
        var vh = Math.min(h, bh - y);
        gr.DrawImage(img, x, y, w, vh, x, y, w, vh, colours.blob);
        gr.FillRectangle(x, y, w, vh,
            grad(0, y, 0, y + vh, setAlpha(colours.bg, 90), colours.bg));
    }
}
...
ArtCard.prototype.paint = function (gr) {
    paintPage(gr, this.ox, this.oy, this.W, this.H);
    ...
    if (!this.count) {
        ...
        if (this.maxScroll() - this.scroll > 1) {
            gr.FillRectangle(x0, y0 + H - fade, W, fade,
                grad(0, y0 + H - fade, 0, y0 + H, setAlpha(colours.bg, 0), colours.bg));
        }
    }
...
BottomBar.prototype.paint = function (gr) {
    ...
    var tx = rc.x + rc.w + px(9), tw = c.x + c.w - px(9) - tx;
    drawText(gr, playing ? evalTf(tf.title, this.metadb) : '재생 중인 항목 없음',
        fonts.small, colours.text, tx, c.y + px(6), tw, px(15), TEXT_LEFT);
    drawText(gr, playing ? evalTf(tf.artist, this.metadb) : '',
        fonts.tiny, colours.sub, tx, c.y + c.h - px(19), tw, px(14), TEXT_LEFT);
...
BottomBar.prototype.move = function (x, y) {
    ...
    if (h !== this.hover) {
        this.hover = h;
        window.SetCursor(h && h !== 'chip' ? IDC_HAND : IDC_ARROW);
        window.Repaint();
    }
};
    ...
    try {
        ...
        for (var i = 0; i < n; i++) {
            menu.AppendMenuItem(MF_STRING, 100 + i,
                plman.GetPlaylistName(i) + '\t' + plman.GetPlaylistItemCount(i));
        }
        if (n > 0) menu.CheckMenuRadioItem(100, 100 + n - 1, 100 + active);
        else menu.AppendMenuItem(MF_STRING, 1, '(재생목록 없음)');
        var id = menu.TrackPopupMenu(x, y);
        if (id >= 100 && id < 100 + n) plman.ActivePlaylist = id - 100;
        return true;
    } catch (e) {
