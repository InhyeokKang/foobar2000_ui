...
var SVG = {
    ...
    search: '<circle cx="10.6" cy="10.6" r="6.1" fill="none" stroke="%C%" stroke-width="1.9"/>' +
            '<path d="M15.2 15.2 L20 20" fill="none" stroke="%C%" stroke-width="2.1" stroke-linecap="round"/>',

    sort: '<g fill="none" stroke="%C%" stroke-width="1.9" stroke-linecap="round">' +
          '<path d="M4 6.5h12"/><path d="M4 12h8"/><path d="M4 17.5h4"/>' +
          '<path d="M17.5 10.5v9"/></g>' +
          '<path d="M14.6 16.9 L17.5 20.4 L20.4 16.9 Z" fill="%C%"/>',

    close: '<g fill="none" stroke="%C%" stroke-width="2" stroke-linecap="round">' +
           '<path d="M7.5 7.5 L16.5 16.5"/><path d="M16.5 7.5 L7.5 16.5"/></g>',
...
function TopBar() {
    ...
    this.buttons = [
        new Button('search',
            function () {
                return [self.searching ? 'close' : 'search',
                        self.searching || self.hover === 'search' ? colours.text : colours.sub];
            },
            function () { self.toggleSearch(); }),
        new Button('gear',
...
TopBar.prototype.toggleSearch = function () {
    this.searching = !this.searching;
    if (!this.searching) { this.query = ''; }
    if (panel.search) panel.search(this.query);
    window.Repaint();
};

TopBar.prototype.type = function (ch) {
...
TopBar.prototype.backspace = function () {
    if (!this.searching || !this.query.length) return false;
...
TopBar.prototype.paint = function (gr) {
    ...
    if (this.searching) {
        var f = this.name, fh = Math.min(px(30), this.H - px(10));
        var fy = this.oy + (this.H - fh) / 2;
        var fw = Math.min(f.w, px(320));
        fillRound(gr, f.x, fy, fw, fh, fh / 2, colours.card);
        drawIcon(gr, 'search', f.x + px(15), fy + fh / 2, px(14), colours.sub);
        var tx = f.x + px(28), tw = fw - px(38);
        if (this.query) {
            drawText(gr, this.query, fonts.row, colours.text, tx, fy, tw, fh, TEXT_LEFT);
            if (this.phase % 26 < 13) {
                var cw = Math.min(textWidth(this.query, fonts.row), tw - px(6));
                gr.FillRectangle(tx + cw + px(2), fy + fh * 0.26, px(1.5), fh * 0.48, colours.sub);
            }
        } else {
            drawText(gr, '제목 · 아티스트 · 앨범 검색', fonts.row, colours.faint, tx, fy, tw, fh, TEXT_LEFT);
        }
        for (var b = 0; b < this.buttons.length; b++) this.buttons[b].paint(gr, 0.62);
        return;
    }
...
TopBar.prototype.tick = function () {
    this.phase++;
    if (this.searching && this.phase % 13 === 0) window.Repaint();
};
...
TrackList.prototype.refresh = function () {
    try {
        ...
        this.all = this.items ? this.items.Count : 0;
    ...
    } catch (e) { this.items = null; this.all = 0; this.name = ''; this.total = 0; }
    this.rows = {};
    this.setQuery(this.query);
};

// The list can show a filtered subset; display index -> playlist index.
TrackList.prototype.at = function (n) {
    return this.filtered ? this.filtered[n] : n;
};

TrackList.prototype.setQuery = function (q) {
    this.query = q || '';
    if (!this.query || this.all > 4000) {          // do not grind a huge playlist
        this.filtered = null;
    } else {
        var lq = this.query.toLowerCase(), out = [];
        for (var i = 0; i < this.all; i++) {
            var r = this.row(i);
            if ((r.title + ' ' + r.artist + ' ' + r.album).toLowerCase().indexOf(lq) >= 0) out.push(i);
        }
        this.filtered = out;
    }
    this.count = this.filtered ? this.filtered.length : this.all;
    this.target = this.scroll = 0;
    this.clampScroll();
    window.Repaint();
};
...
TrackList.prototype.paint = function (gr) {
    ...
    if (this.headerH) {
        ...
        var sub;
        if (this.query) {
            sub = '"' + this.query + '"  ·  ' + this.count + ' / ' + this.all + ' Songs';
        } else {
            sub = this.count + ' Songs';
    ...
    if (!this.count) {
        drawText(gr, this.dropActive ? '여기에 놓으면 추가됩니다'
                   : (this.query ? '검색 결과가 없습니다' : '재생목록이 비어 있습니다'),
        ...
        for (var n = first; n <= last; n++) this.paintRow(gr, n, playingIdx);
...
TrackList.prototype.paintRow = function (gr, n, playingIdx) {
    var y = this.top + n * this.rowH - this.scroll;
...
TrackList.prototype.down = function (x, y, mask) {
    ...
    try {
        if (ctrl) {
            ...
            for (var k = a; k <= b; k++) {
                if (!this.filtered || this.displayPos(k) >= 0) idx.push(k);
            }
...
TrackList.prototype.displayPos = function (i) {
    if (!this.filtered) return i;
    for (var n = 0; n < this.filtered.length; n++) if (this.filtered[n] === i) return n;
    return -1;
};

TrackList.prototype.dblclick = function (x, y) {
    var n = this.rowAt(y);
    if (n >= 0) { try { plman.ExecutePlaylistDefaultAction(this.pl, this.at(n)); } catch (e) {} }
};
...
TrackList.prototype.key = function (vkey) {
    ...
    if (vkey === VK_UP || vkey === VK_DOWN) {
        var pos = this.displayPos(focus);
        if (pos < 0) pos = 0;
        var nextPos = clamp(pos + (vkey === VK_DOWN ? 1 : -1), 0, this.count - 1);
        var next = this.at(nextPos);
        try {
            plman.ClearPlaylistSelection(this.pl);
            plman.SetPlaylistSelectionSingle(this.pl, next, true);
            plman.SetPlaylistFocusItem(this.pl, next);
        } catch (e) {}
        this.ensureVisible(nextPos);
...
function BottomBar() {
    ...
    this.buttons = [
        new Button('sort',
            function () { return ['sort', self.hover === 'sort' ? colours.text : colours.sub]; },
            function (x, y) { showSortMenu(x, y); })
    ];
}
...
Composite.prototype.search = function (q) { this.list.setQuery(q); };
Composite.prototype.typeChar = function (ch) { return this.top.type(ch); };
Composite.prototype.backspace = function () { return this.top.backspace(); };
Composite.prototype.searching = function () { return this.top.searching; };
Composite.prototype.closeSearch = function () {
    if (this.top.searching) { this.top.toggleSearch(); return true; }
    return false;
};
...
// Sorting the playlist in place — the one thing the bottom right corner
// could not do before. Every sort is backed up first, so Edit > Undo works.
var SORTS = [
    ['제목',      '%title%'],
    ['아티스트',  '%artist%|%album%|%tracknumber%'],
    ['앨범',      '%album%|%tracknumber%'],
    ['길이',      '%length_seconds%'],
    ['파일 경로', '%path%']
];
...
function showSortMenu(x, y) {
    ...
    try {
        ...
        for (var i = 0; i < SORTS.length; i++) {
            menu.AppendMenuItem(MF_STRING, 10 + i, SORTS[i][0] + ' 순');
        }
        menu.AppendMenuSeparator();
        menu.AppendMenuItem(MF_STRING, 50, '무작위로 섞기');
        var id = menu.TrackPopupMenu(x, y);
        if (id <= 0) return true;
        var pl = plman.ActivePlaylist;
        if (pl < 0) return true;
        plman.UndoBackup(pl);
        if (id === 50) plman.SortByFormat(pl, '$rand()');
        else if (id >= 10 && id < 10 + SORTS.length) plman.SortByFormat(pl, SORTS[id - 10][1]);
        return true;
    } catch (e) {
        console.log('Studio: 정렬 실패 - ' + e);
        return false;
    } finally {
...
var VK_CONTROL = 0x11, VK_P = 0x50, VK_BACK = 8, VK_ESCAPE = 27;
...
function on_key_down(vkey) {
    ...
    if (panel.searching && panel.searching()) {
        if (vkey === VK_BACK) { panel.backspace(); return; }
        if (vkey === VK_ESCAPE) { panel.closeSearch(); return; }
    }
    if (panel.key) panel.key(vkey);
}

// Typing goes to the search field while it is open.
function on_char(code) {
    if (code >= 32 && panel.typeChar) {
        try { if (panel.typeChar(String.fromCharCode(code))) return; } catch (e) {}
    }
}
